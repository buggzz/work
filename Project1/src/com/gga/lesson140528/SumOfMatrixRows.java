...
//		}
		...
		double[][] matrix = generateMatrix(10, 10);
		long start = System.nanoTime();
		double sum;
        sum = sum(matrix);
		long stop = System.nanoTime();
		long elapsed = stop - start;
		System.out.println("sum : " + sum + " " + elapsed);
			...
			threads[i] = new Thread(new Runnable() {
				...
				public void run() {
					...
					for (int j = 0; j < row.length; j++) {
						...
						rowSum += Math.sin(Math.pow(row[j], Math.E))/Math.cos(Math.pow(row[j], Math.E));
					}
					sum[rowIndex] = rowSum;
				}
			});
	...
	private static double calculate(double[][] matrix, int i, int j) {
		return Math.sin(Math.pow(matrix[i][j], Math.E))/Math.cos(Math.pow(matrix[i][j], Math.E));
	}
